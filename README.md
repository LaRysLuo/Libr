...
Libr 是一个面向 Windows 10/11 与 macOS 12+ 的本地资源管理软件。资源可以映射原文件以节省空间，也可以连同内容保存在一个可携带的 `.libr` 文件中。

## 当前实现

- Tauri 2 + React 19 + TypeScript + Rust，简体中文三栏桌面界面。
- 创建、打开、另存副本、双击文件关联和每库单写实例。
- 文件/文件夹批量导入、任务进度与取消、SHA-256 精确去重；默认映射原文件以节省空间，也可选择复制或剪切进资源库。
- 文件夹、标签、智能文件夹、收藏、评分、颜色、备注、搜索、组合筛选、回收站、导出和外部打开。
- 可将指定文件夹（含子文件夹）临时分享到局域网；浏览器端支持预览、下载，以及可选的重命名、收藏和移到回收站操作。
- 图片缩略图与主色提取，支持常规图片及 iPhone HEIC/HEIF/HIF；MP4/MOV 视频优先使用内嵌封面，无封面时显示代表性视频帧；原文件通过受限 `libr://` Range 协议读取。
...
Office、PSD、AI、Sketch 等专有格式首版不做应用内解析，会显示类型预览并支持外部打开。首版也不包含账号、同步、协作、AI/OCR、近似图片搜索或密码加密。
...
资源按 SHA-256 在 `blobs` 中唯一登记，`assets` 只引用数据记录。映射模式保存原文件绝对路径和缩略图，移动或删除原文件会使资源暂时不可用；复制/剪切模式把原始内容写入 BLOB，可跨设备独立使用。遇到高于当前应用支持的架构版本时，资源库以只读方式打开。
